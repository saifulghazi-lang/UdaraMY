...
const points = computed(() => {
  ...
  return props.history.map((pt, idx) => {
...
<template>
  <div class="rounded-3xl p-5 sm:p-6 bg-black border border-white/[0.12] shadow-2xl space-y-3">
    ...
    <div class="flex items-center justify-between">
      ...
      <!-- Controls: Timelapse Play + 24h Delta Pill -->
      <div class="flex items-center gap-2">
        <!-- Interactive Timelapse Player Button -->
        <button
    ...
    <div class="flex items-center justify-between p-2.5 rounded-xl bg-black border border-white/10 text-xs font-mono">
      <div class="flex items-center gap-2">
        <span class="text-slate-400">Scrub Time:</span>
        <span class="text-white font-bold">{{ activePoint?.timeLabel || 'Now' }}</span>
      </div>
      <div class="flex items-center gap-2">
